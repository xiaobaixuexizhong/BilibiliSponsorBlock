...
- `page`：页面基本信息
- `counts`：各类日志条数
- `lifecycleSummary`：按 `stage` 聚合后的摘要，会持续累计
- `logs`：最近一段原始日志明细（有上限，避免无限增长）

只有在打开调试开关后，生命周期日志才会额外输出到控制台，便于现场盯时序。
